...
class ServicosProdutosController extends Controller
{
  ...
  public function fornecedoresProduto($id)
  {
    $products = ProviderProduct
      ::where('id_fornecedor', '=', $id)
      ->with('idp8nnq72l.ecgklyqfdcoguyj')
      ->paginate(30);
  ...
  public function changeProviderProduct(Request $request)
  {
    if ($request->change == 'detach')
    {
      $nomeProduto = ServiceProduct::find($request->id)->nome;
      $people  = ServiceProduct::find($request->id)->i8tk3ymx5u()->detach([$request->provider]);

      $response = [
        'type'     => 'warning',
        'message'  => "{$nomeProduto} removido do fornecedor",
      ];
    }
    else if ($request->change == 'syncWithoutDetaching')
    {
      $nomeProduto = ServiceProduct::find($request->id)->nome;
      $people  = ServiceProduct::find($request->id)->i8tk3ymx5u()->syncWithoutDetaching([$request->provider]);

      $response = [
        'type'     => 'success',
        'message'  => "{$nomeProduto} adicionado no fornecedor",
      ];
    }

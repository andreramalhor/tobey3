...
{
  public function a_receber()
  {
    return view('sistema.financeiro.a_receber.index');
  }

  public function a_receber_tabelar(Request $request)
  {
    $a_receber = AReceber::
                          whereDate('dt_vencimento', '=', $request->dt_consulta ?? \Carbon\Carbon::today())->
                          where('status', '=', null)->
                          orderBy('nome', 'asc')->
                          get();

    return view('sistema.financeiro.a_receber.tabelar', [
      'a_receber' => $a_receber,
    ]);
  }

  public function a_receber_selecionar($id)
  {
    $a_receber = AReceber::find($id);

    return $a_receber;
  }

  public function a_receber_confirmar(Request $request)
  {
    // $selecionados = json_decode(request['selecionados'], true);
    // $teste        = collect($selecionados);
    $fin_comissoes   = collect(json_decode($request['selecionados'], true));

    foreach($fin_comissoes as $conta )
    {
      $a_receber = AReceber::find($conta['id']);
      $a_receber->tipo                   = 'Parcela';
      $a_receber->forma_pagamento        = $request['forma_pagamento'] ?? null;
      $a_receber->tarifa                 = $request['tarifa'] ?? null;
      $a_receber->vlr_final              = $request['vlr_final'] ?? null;
      $a_receber->dt_pagamento           = $request['dt_recebimento'] ?? null;
      $a_receber->dt_lancamento          = $request['dt_recebimento'] ?? null;
      $a_receber->status                 = 'Confirmado';
      $a_receber->id_banco               = $request['id_banco'] ?? null;
      $a_receber->forma_receb            = $request['forma_receb'] ?? null;
      $a_receber->dt_recebimento         = $request['dt_recebimento'] ?? null;
      $a_receber->update();
    }

    // if ( !empty( json_decode($request->selecionados) ) )
    //   {
    //     foreach ( json_decode($request->selecionados, true) as $atd_address )
    //     {
    //       return 11333;
    //       $pessoa->uqbchiwyagnnkip()->create($atd_address);
    //     }
    //   }
    ...
    // $someJSON = [$request->selecionados];
    // // Convert JSON string to Array
    // $someArray = json_decode($someJSON, true);

    // // return $someJSON;
    // return $someArray;
    // $selecionados = collect(json_decode($request->selecionados, true));


    // return $selecionados;

    // $selecionados = collect(json_decode($request->selecionados, true));
    // foreach($selecionados as $key => $conta_a_receber)
    // {
    //   return $key;
    // }
  ...
  public function a_rebecer_por_aluno($id)
  {
    $a_receber = AReceber::
                          where('id_pessoa', '=', $id)->
                          get();
  ...
  {
    switch ($request->tipo_tabela)
    {
      case 'confirmado':
        $lancamentos = $this->lancamentos_tabelar_confirmados();
        break;
      
      case 'a_confirmar':
        $lancamentos = $this->lancamentos_tabelar_nao_confirmados();
        break;

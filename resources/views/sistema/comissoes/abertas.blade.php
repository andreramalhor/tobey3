  ...
  <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
    <div class='card card-primary'>
      <div class='card-header'>
        <h3 class='card-title'>Comissões Abertas: {{ Auth::User()->apelido ?? $abertas->first()->xeypqgkmimzvknq->apelido }}</h3>
      </div>
                      ...
                      <tbody>
                        @foreach($comissoes->sortBy('dt_prevista') as $aberta)
                          ...
                          <tr data-widget='expandable-table'>
                            <td width="11%" class='text-center p-0 border-0' colspan="2">{{ $aberta->id }}</td>
                            ...
                            @if($aberta->fonte_origem == 'pdv_vendas_pagamentos' )
                              <td width="5%" class='text-center p-0 border-0'><a href="" data-bs-toggle="modal" onclick="vendas_mostrar_modal({{ $aberta->sflfmensjwleneb->id_venda ?? 'ERROR 562542' }})"><span class="badge bg-primary">{{ $aberta->sflfmensjwleneb->id_venda ?? 'ERROR 562542' }}</span></td>
                            @elseif($aberta->fonte_origem == 'pdv_vendas_detalhes' )
                              <td width="5%" class='text-center p-0 border-0'><a href="" data-bs-toggle="modal" onclick="vendas_mostrar_modal({{ $aberta->skfmwuorwmlpdlm->id ?? 'ERROR 562542' }})"><span class="badge bg-pink">{{ $aberta->skfmwuorwmlpdlm->id ?? 'ERROR 562542' }}</span></td>
                            ...
                            @elseif($aberta->fonte_origem == 'fin_conta_interna' )
                              <td width="5%" class='text-center p-0 border-0'>{{ $aberta->tipo }}</td>
                            @else
                              <td width="5%" class='text-center p-0 border-0'>{{ $aberta->tipo }}</td>
                            @endif
                            ...
                            @if($aberta->fonte_origem == 'pdv_vendas_pagamentos' )
                              <td width="16%" class='text-center p-0 border-0'>{{ $aberta->tipo }}</td>
                            @elseif($aberta->fonte_origem == 'pdv_vendas_detalhes' )
                              <td width="16%" class='text-left p-0 border-0'>{{ $aberta->skfmwuorwmlpdlm->lufqzahwwexkxli->apelido ?? $aberta->skfmwuorwmlpdlm->id_cliente  ?? $aberta->id_origem ?? 'ERROR 256635' }}</td>
                            @elseif($aberta->fonte_origem == 'fin_lancamentos' )
                            <td width="16%" class='text-left p-0 border-0'> - </td>
                            @elseif($aberta->fonte_origem == 'fin_conta_interna' )
                            <td width="16%" class='text-left p-0 border-0'>{{ $aberta->fonte_origem }}</td>
                            @else
                              <td width="16%" class='text-left p-0 border-0'> - </td>
                            @endif
                            ...
                            @if($aberta->fonte_origem == 'pdv_vendas_pagamentos' )
                              ...
                              @endif
                            @elseif($aberta->fonte_origem == 'pdv_vendas_detalhes' )
                            <td width="" class='text-left p-0 border-0'>{{ $aberta->ygferchrtuwewhq->nome ?? $aberta->id_origem ?? 'ERROR 745214' }}</td>
                            @elseif($aberta->fonte_origem == 'fin_lancamentos' )
                            <td width="" class='text-left p-0 border-0'>{{ $aberta->tipo }}</td>
                            @elseif($aberta->fonte_origem == 'fin_conta_interna' )
                            <td width="" class='text-left p-0 border-0'>{{ $aberta->fonte_origem }}</td>
                            @else
                              <td width="" class='text-left p-0 border-0'> - </td>
                            @endif
  
                            <td width="6%" class='text-center p-0 border-0'>{{ number_format( $aberta->percentual * 100, 0) }} %</td>
...
@include('includes.catch', [ 'codigo_erro' => '2643332a' ] )
    .then( function(response)
    {
...
@include('includes.catch', [ 'codigo_erro' => '8495533a' ] )
    .then( function(response)
    {
      comissoes_tabelar_abert_prof()
    })
...
@include('includes.catch', [ 'codigo_erro' => '7658380a' ] )
    .then( function(response)
    {
      comissoes_tabelar_abert_prof()
    })

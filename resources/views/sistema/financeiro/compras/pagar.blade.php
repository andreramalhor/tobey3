...
@section('content')
<form method="POST" action="{{ route('compra.registrar') }}" id="form_compra_create" autocomplete="off">
  @csrf
  <input type="hidden" name="fin_comanda" id="fin_comanda" value={{ json_encode($compra) }}>
  <input type="hidden" name="fin_comanda_detalhes" id="fin_comanda_detalhes" value={{ json_encode($compra->FinanceiroComprasDetalhesProdutos) }}>
  <input type="hidden" name="fin_comanda_pagamentos" id="fin_comanda_pagamentos">
  <div class="row">
    <div class="col-7" id="box_fornecedor">
      <div class="card">
        ...
        <div class="card-body" style="padding-top: 0px">
          <div class="row">
            <div class="col-2">
              ...
              <input type="text" class="form-control form-control-sm" value="{{ $compra->id_fornecedor ?? '#id' }}" readonly="readonly">
            </div>
            <div class="col-10">
              <label class="col-form-label">Nome</label>
              <input type="text" class="form-control form-control-sm" value="{{ $compra->AtdPessoasFornecedoresCompras->nome ?? $compra->id_fornecedor ?? 'Nome Fornecedor' }} ( {{ $compra->AtdPessoasFornecedoresCompras->apelido ?? '' }} )" readonly="readonly">
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body" style="padding-top: 0px">
          <div class="row">
            <div class="col-4">
              ...
              <input type="text" class="form-control form-control-sm text-right" id="b_vlr_total" value="R$ {{ number_format($compra->FinanceiroComprasDetalhesProdutos->sum('vlr_final'),2,",",".") }}" readonly="readonly">
            </div>
            <div class="col-4">
            ...
            <div class="col-4">
              ...
              <input type="text" class="form-control form-control-sm text-right" id="b_vlr_restante" value="R$ {{ number_format($compra->FinanceiroComprasDetalhesProdutos->sum('vlr_final'),2,",",".") }}" readonly="readonly">
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="row">
        ...
        <div class="card-body" style="padding-top: 0px">
          <div class="row">
            ...
            <div class="col-5">
              ...
              <input type="text" class="form-control form-control-sm text-right" id="valor" value="{{ number_format($compra->FinanceiroComprasDetalhesProdutos->sum('vlr_final'),2,",",".") }}" onFocus="this.selectionStart = this.selectionEnd = this.value.length;" readonly="readonly">
            </div>
          </div>
        </div>
    ...
    <div class="col-9" id="box_pagamentos">
      <div class="card">
        ...
        <div class="card-footer">
          <a class="btn btn-danger btn-sm" href="{{ route('compra.index') }}">Cancelar</a>
...
<script type="text/javascript">
  ...
  fin_comanda_totais                = {};
  fin_comanda_totais.total_pedido   = parseFloat({{ $compra->FinanceiroComprasDetalhesProdutos->sum('vlr_final') }});
  fin_comanda_totais.total_lancado  = 0;
  fin_comanda_totais.vlr_restante   = fin_comanda_totais.total_pedido - fin_comanda_totais.total_lancado;
  fin_comanda_totais.troco          = 0;
  fin_comanda_totais.vale           = 0;
  fin_comanda_totais.credito_casa   = 0;

  fin_comanda_pagamentos = [];
  ...
  function ajustar_valor(e) {
    ...
    if (e == undefined)
    {
      $("#valor").attr({"readonly": false}).val(parseFloat(fin_comanda_totais.vlr_restante).toFixed(2).replace('.', ',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.').toString()).focus();
    }
    else
    {
...
function adicionar()
{
  ...
  else
  {
    for( i=0 ; i < todosPagamentos[0].parcela ; i++ )
    {
      ...
      fin_comanda_pagamentos.push(pagamento);
    }
    indice++;
  }
  atualizarTabela();
...
{
  item = fin_comanda_pagamentos.findIndex(val => val.identificacao == index);
    // fin_comanda_pagamentos.splice(item, 1);
...
{
  let dinheiro        = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Dinheiro').reduce((anterior, atual) => anterior + atual.valor, 0)
  let cartao_debito   = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Cartão de Débito').reduce((anterior, atual) => anterior + atual.valor, 0)
  let cartao_credito  = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Cartão de Crédito').reduce((anterior, atual) => anterior + atual.valor, 0)
  let cheque          = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Cheque').reduce((anterior, atual) => anterior + atual.valor, 0)
  let deposito        = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Depósito').reduce((anterior, atual) => anterior + atual.valor, 0)
  let fiado           = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Fiado').reduce((anterior, atual) => anterior + atual.valor, 0)
  let credito_casa    = fin_comanda_pagamentos.filter((dados) => dados.forma_pagamento === 'Conta Interna').reduce((anterior, atual) => anterior + atual.valor, 0);

  fin_comanda_totais.total_lancado  = fin_comanda_pagamentos.reduce((anterior, atual) => anterior + atual.valor, 0);
  fin_comanda_totais.vlr_restante   = fin_comanda_totais.total_pedido - fin_comanda_totais.total_lancado;
  ...
  // console.log('----------------------------')
  // console.log(fin_comanda_totais)
  // console.log('============================')


  //   if(fin_comanda_totais.total_pedido >= fin_comanda_totais.total_lancado) {
  //     $("#troco").val(reais(0));
  //   } else {
  //     $("#troco").val(reais(fin_comanda_totais.total_lancado - fin_comanda_totais.total_pedido));
  //   }

  $("#b_vlr_total").val(reais(fin_comanda_totais.total_pedido));
  $("#b_vlr_lancado").val(reais(fin_comanda_totais.total_lancado));
  $("#b_vlr_restante").val(reais(fin_comanda_totais.vlr_restante));

  if ( fin_comanda_totais.vlr_restante < 0.01 && fin_comanda_totais.vlr_restante > -0.01 ) {
    $("#btn_finalizar").show();
  } else {
    $("#btn_finalizar").hide();
  }

  var JSONString = JSON.stringify(fin_comanda_pagamentos);
  $("#fin_comanda_pagamentos").val(JSONString);
}
...
function atualizarTabela()
{
  ...
  $.each( fin_comanda_pagamentos, function( key, pagamento ) {
...
@include('includes.catch', [ 'codigo_erro' => '4725475a' ] )
.then( function() {
  setTimeout(function() {

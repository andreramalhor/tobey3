    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-8">
            <input type="text" class="form-control form-control-sm text-right n_decimal" id="N_vlr_custo[{{ $produto->id }}]" value="{{ number_format(0,2,",",".") }}" onFocus="this.selectionStart = this.selectionEnd = this.value.length;">
          </div>
        </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-8">
            <input type="text" class="form-control form-control-sm text-right n_decimal" id="N_vlr_venda[{{ $produto->id }}]" value="{{ number_format(0,2,",",".") }}" onFocus="this.selectionStart = this.selectionEnd = this.value.length;">
          </div>
        </div>
      </div>
      <div class="modal-footer justify-content-between">
    ...
    <table class="table projects table-bordered">
      ...
      <tfoot>
        <tr>
          ...
          <td class="text-center p-1">
            <span id="total_qtd">0</span>
          </td>

          <td colspan="2"></td>

          <td class="text-center p-1">
            <span id="total_itens">0</span>

          <td class="text-right p-1">
            <a>
              <span id="total_custo">0,00</span>
            </a>
            <br>
            <small>
              <span id="total_lucro">0,00</span>
            </small>
          </td>
...
$(document).ready(function()
{
  ...
  function menos( id )
  {
    produto_x = dados_produtos.filter( elem => parseInt(elem.id) === parseInt(id) ).reduce(function getTotal(total, item)
    {
      if (item.N_qtd != 0)
      {
  ...
  function atualizar_valores( id )
  {
    ...
    produto_x = dados_produtos.filter( elem => parseInt(elem.id) === parseInt(id) ).reduce(function getTotal(total, item)
    {
      ...
      total_qtd   = dados_produtos.reduce( (acumulador, data) => data.N_qtd != undefined && data.N_qtd != 0  ? acumulador + 1 : acumulador , 0 )
      total_itens = dados_produtos.reduce( (acumulador, data) => data.N_qtd != undefined ? acumulador + data.N_qtd : acumulador , 0 )
      total_custo = dados_produtos.reduce( (acumulador, data) => data.N_qtd != undefined ? acumulador + ( data.N_vlr_custo * data.N_qtd ) : acumulador , 0 )
      total_lucro = dados_produtos.reduce( (acumulador, data) => data.N_qtd != undefined ? acumulador + ( ( data.N_vlr_venda * data.N_qtd ) - ( data.N_vlr_custo * data.N_qtd ) ) : acumulador , 0 )


      $('#total_qtd').html(total_qtd);
      $('#total_itens').html(total_itens);
      $('#total_custo').html(accounting.formatMoney(total_custo, ""));
      $('#total_lucro').html(accounting.formatMoney(total_lucro, ""));

      $('#total_qtd_compra').val(total_qtd);
      $('#total_itens_compra').val(total_itens);
      $('#total_custo_compra').val(accounting.formatMoney(total_custo, ""));
      $('#total_custo_compra').val(accounting.formatMoney(total_custo, ""));
      $('#total_custo_compra').val(accounting.formatMoney(total_custo, ""));

      $("#dados_produtos").val(JSON.stringify(dados_produtos))

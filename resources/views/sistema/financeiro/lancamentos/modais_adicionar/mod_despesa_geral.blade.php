<form autocomplete='off' id='form-lancamentos-despesa_geral'>
  <div class='modal-dialog modal-xl'>
    <div class='modal-content'>
      <div class="overlay" id="mod_despesa_geral-overlay">
        <i class="fas fa-2x fa-sync fa-spin"></i>
      </div>
      <div class='modal-header bg-navy' style='padding: 8px 16px'>
        <h5 class='modal-title'>Despesas Gerais</h5>
      </div>
      ...
      <div class='modal-footer justify-content-between' style='padding: 6px 12px'>
        ...
        <a class='btn btn-primary' id='submit-despesa_geral' style='display: none;'>Efetivar</a>
      </div>
    </div>
  </div>
  ...
  <input type='hidden' name='0[dt_lancamento]'          value='{{ \Carbon\Carbon::today() }}'>
  ...
  <input type='hidden' name='tipo'                      value='despesa_geral'>
</form>
...
function contas_contabeis()
{
  ...
  .then(function(response)
  {
    ...
    collect(response.data).each((value, key) => {
      $("[name='0[id_conta]']").append('<option value="'+value.id+'">'+value.nova_conta+' - '+value.titulo+'</option>');
    });
  })
...
function bancos_plucar()
{
  ...
  .then( function()
  {
    ...
    setTimeout(() => {
      $('#mod_despesa_geral-overlay').hide()
    }, 500);
  })
}

$('#form-lancamentos-despesa_geral').change(function()
{
  ...
  {
    $('#submit-despesa_geral').hide()
  }
  else
  {
    $('#submit-despesa_geral').show()
  }
})

$('#submit-despesa_geral').click(function(event)
{
  $('#mod_despesa_geral-overlay').show()
  
  event.preventDefault()

  dados = $('#form-lancamentos-despesa_geral').serialize()

    ...
    <div class="card">
      <div class="card-body box-profile">
        <div class="text-center">
          <img class="border border-secondaty rounded" src="{{ $produto->imagem }}" alt="{{ $produto->nome }}" width="200px">
        </div>
        ...
        <span class="text-center">
          <strong><i class="fa-solid fa-font-awesome"></i> Tipo</strong>
            <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8"> {{ $produto->tipo }} </font></p>
        </span>
        <span class="text-center">
          <strong><i class="fa-solid fa-box-archive"></i> Categoria</strong>
            <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8"> {{ $produto->ecgklyqfdcoguyj->nome ?? $produto->id_catgoria ?? '(Não categorizado)' }} </font></p>
        </span>
        <span class="text-center">
          <strong><i class="fa-regular fa-copyright"></i> Marca</strong>
            <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8"> {{ $produto->marca }} </font></p>
        </span>
        <span class="text-center">
          <strong><i class="fa-solid fa-money-bill"></i> Valor de Venda</strong>
            <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">R$ {{ number_format($produto->vlr_venda, 2, ',', '.') }} </font></p>
        </span>
        <span class="text-center">
          <strong><i class="far fa-id-card mr-1"></i> {{ $produto->label_cpf_cnpj }} </strong>
          <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ $produto->cpf ?? "-" }}</font></p>
        </span>
        <span class="text-center">
          <strong><i class="far fa-file-alt mr-1"></i> Observação</strong>
          <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ $produto->observacao ?? "-" }}</font></p>
        </span>
        @can('Equipe.Alterar Senha')
          @if(isset($produto->krdhcnrxogfuwla) AND $produto->id == Auth::User()->id)
            <a href="{{ route('atd.equipe.alterar_senha', $produto->id) }}" class="btn btn-primary btn-block"><b>Alterar Senha</b></a>
          @endif
        @endcan
      </div>
    </div>
...
{{--
  @forelse($produto->ginthgfwxbdhwtu->sortbyDesc('whatsapp') as $contato)
  ...
  <strong><i class="fas fa-map-marker-alt mr-1"></i> Localização</strong>
        @forelse($produto->uqbchiwyagnnkip as $endereco)
          <p class="text-muted" style="margin-bottom: 2px"><font size="2"> 
            <span style="font-size: 7px;"><i class="fas fa-circle fa-xs"></i></span>
 ...
 --}}
        ...
        @if(isset($produto->instagram) || isset($produto->facebook))
          <p class="text-muted" style="margin-bottom: 2px"><font size="2"> 
            <span style="font-size: 7px;"></span>
            @if($produto->instagram)
              <a class="btn btn-default btn-xs" href="https://www.instagram.com/{{ $produto->instagram }}" target="_black" data-bs-tt="tooltip" title="Instagram"><i class="fa-brands fa-instagram"></i></a> {{ $produto->instagram }}
            @endif
            <br>
            @if($produto->facebook)
              <a class="btn btn-default btn-xs" href="https://www.facebook.com/{{ $produto->facebook }}" target="_black" data-bs-tt="tooltip" title="Facebook"><i class="fa-brands fa-facebook"></i></a> {{ $produto->facebook }}
            @endif
        @else
      ...
      <div class="card-body">
        <div class="tab-content">
          <div class="tab-pane active" id="painel">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_painel')
          </div>
          <div class="tab-pane" id="vendas">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_vendas')
          </div>
          <div class="tab-pane" id="compras">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_compras')
          </div>
          ...
          <div class="tab-pane" id="sobre">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_sobre')
          </div>
          <div class="tab-pane" id="tipo">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_tipo')
          </div>
          <div class="tab-pane" id="fornecedores">
            @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_fornecedores')
          </div>
          <div class="tab-pane" id="comissoes">
          @include('sistema.catalogo.produtos.auxiliares.inc_mostrar_comissoes')
          </div>
        </div>
      </div>
    </div>
    ...
    collect(response.data.data).each((data) =>
    {
      $("#produtosMostrarVendas").append(
        '<tr class="'+(data.deleted_at != null ? 'bg-danger' : '' )+'">'+
          '<td width="5%" style="padding: 0px 1px" class="text-center"><a href="" data-bs-toggle="modal" onclick="showVenda('+data.id_venda+')"><span class="badge bg-primary">'+data.id_venda+'</span></a></td>'+
...
@include('includes.catch', [ 'codigo_erro' => '4659983a' ] )
  .then(function(response)
  {
  ...
  .then(function(response)
  {
    collect(response.data.data).sortByDesc('id_compra').each((data) =>
    {
      $("#produtosMostrarCompras").append(
        '<tr class="'+(data.deleted_at != null ? 'bg-danger' : '' )+'">'+
          '<td width="5%" style="padding: 0px 1px" class="text-center"><a href="" data-bs-toggle="modal" onclick="showVenda('+data.id_compra+')"><span class="badge bg-primary">'+data.id_compra+'</span></a></td>'+
...
@include('includes.catch', [ 'codigo_erro' => '9468716a' ] )
  .then(function(response)
  {

<div class="modal fade" id="modal_pessoas_contato" aria-hidden="true" style="display: none; color: black;">
  <form autocomplete="off" id="form-contato-adicionar">
    @csrf
    ...
    @if (isset($pessoa))
      ...
      var contato = {
        ...
        'acoes'           : '<div class="btn-group">'+
          '<a onclick="contato_excluir('+({{ $loop->index + 1 }})+');" class="btn btn-default btn-xs" data-tt="tooltip" title="Remover"><i class="fas fa-trash-alt"></i></a>'+
        '</div>',
      };
...
$("#submit_pessoas_contato").on('click', function (e)
{
  ...
  var contato = {
    ...
    'acoes'           : '<div class="btn-group">'+
    '<a onclick="contato_excluir('+(contato_indice() + 1)+');" class="btn btn-default btn-xs" data-tt="tooltip" title="Remover"><i class="fas fa-trash-alt"></i></a>'+
    '</div>',
  };
...
function contatos_atualizar_tabela()
{
  ...
  else
  {
    pessoas_contatos.forEach((obj, i) => {
      ...
      if (obj.whatsapp)
      {
        var zap = '<a href="https://api.whatsapp.com/send?phone=55'+obj.ddd+(obj.telefone).replace(/\D/g, '')+'"  target="_black" class="btn btn-default btn-xs" data-tt="tooltip" title="WhatsaApp"><i class="fab fa-whatsapp"></i></a>';
      }
      else
      {
        var zap = '';

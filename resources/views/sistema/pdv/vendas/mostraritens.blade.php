<table id="table-detalhes" class="table table-hover table-bordered table-condensed table-sm">
  ...
  <tbody id="campos_contatos">
    ...
    <tr>
      <td class="text-right">{{ number_format(($item->ContaInternaLigada->percentual ?? 0) * 100, 1, ',', '.') }} %</td>
      <td class="text-right">{{ number_format(($item->ContaInternaLigada->valor ?? 0), 2, ',', '.') }}</td>
      <td class="text-left">{{ $item->ContaInternaLigada->DetalhesDoProfissionalQueFezEssaVenda->apelido ?? 'Profissional' }}</td>
      
      <td class="text-left">{{ $item->PDVServicoProdutosVendasDetalhes->nome ?? 'Prod / Serv' }}</td>
  ...
  <tfoot>
    <tr>
      <th class='text-right'>{{ number_format($itens->sum('vlr_final') - $itens->sum('ContaInternaLigada.valor'), 2, ',', '.') }}</th>
      <th class='text-right'>{{ number_format($itens->sum('ContaInternaLigada.valor'), 2, ',', '.') }}</th>

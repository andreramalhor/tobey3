<div class="card-body table-responsive p-0">
  ...
  --}}
          <td class="text-nowrap text-center">
            @can('Vendas.Detalhes')
              <a href="{{ route('pdv.vendas.mostrar', $venda) }}" class="text-muted" data-tt="tooltip" title="Visualizar" data-original-title="Visualizar"><i class="fa-solid fa-eye"></i></a>&nbsp;&nbsp;
            @endcan
{{-- 
  @can('Vendas.Editar')
  <a href="{{ route('pdv.vendas.editar', $venda->id) }}" class="text-muted" data-tt="tooltip" title="Editar" data-original-title="Editar"><i class="fa-solid fa-pencil"></i></a>&nbsp;&nbsp;
  @endcan
  --}}
            
            @can('Vendas.Excluir')
              @if($venda->deleted_at == null)
                <a onClick="vendas_excluir({{$venda->id}})" class="text-muted" data-tt="tooltip" title="Excluir" data-original-title="Excluir"><i class="fa-solid fa-trash"></i></a>
              @else
                <a onClick="vendas_restaurar({{$venda->id}})" class="text-muted" data-tt="tooltip" title="Reativar" data-original-title="Reativar"><i class="fa-solid fa-recycle"></i></a>
              @endif
            @endcan
          </td>

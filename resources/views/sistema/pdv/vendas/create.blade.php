  ...
  <div class="row">
    <div class="col-4">
      <div class="row">
          ...
          <div class="card">
            ...
            <div class="card-header">
              ...
              <div class="card-tools">
                <div class="btn-group">
                  <a class="btn btn-sm btn-default" data-toggle="modal" id="btn_modal_sobreOCliente" href="#modal_sobreOCliente" style="display: none;"><i class="fas fa-fw fa-address-card"></i></a>
                  <a class="btn btn-sm btn-default" data-toggle="modal" id="btn_modal_sobreAComanda" href="#modal_sobreAComanda" style="display: none;"><i class="fas fa-fw fa-search"></i></a>
        ...
        {{-- ====================================================================================================================================================================================================== --}}
        <div class="col-12" id="escolher_produto_servico" style="display: none;">
          <div class="card">
            ...
            <div class="card-header">
              ...
              <div class="card-tools">
                <div class="btn-group">
                  <a class="btn btn-sm btn-default" data-toggle="modal" href="#modal_sobreOProdutoServico" id="m2_sobreOProdutoServico" style="display: none;"><i class="fas fa-fw fa-search"></i></a>
                  <a class="btn btn-sm btn-default" data-toggle="modal" href="#modal_agendados" id="btn_modal_agendados" style="display: none;"><i class="fas fa-fw fa-calendar-alt"></i></a>
                </div>
              </div>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-12">
                  ...
                  <select class="form-control form-control-sm" name="id_produto_servico" id="id_produto_servico" onchange="selecionarProdutoServico(this.value)">
                    <option value="">Selecione . . .</option>
                    @foreach($produtos_servicos as $categoria => $produtos_servicos)
                    <optgroup label="{{ $categoria }}">  
                      @foreach($produtos_servicos as $produto_servico)
                      <option value="{{ $produto_servico->id }}">{{ $produto_servico->nome }}</option>
                      @endforeach
                    </optgroup>
                    @endforeach
                  </select>
            ...
            <div class="card-header">
              ...
              <div class="card-tools">
                <div class="btn-group">
                  ...
                  <a class="btn btn-sm btn-default" data-toggle="modal" data-target="#modal_pesquisa" id="modal_pesquisa_modalzim"><i class="fas fa-filter"></i></a>
                </div>
              </div>
            </div>
            <div class="card-body" style="padding: 5px">
              <div class="row">
                <div class="col-12 radio-group" id="profisionais_radio">
                  <img src="{{ asset('img/Atendimentos/Pessoas/Perfil/0.png') }}" class="img-circle radio" alt="User Image" width="50px" style="margin: 5px;"
                  id="idProfissional_" data-value="" data-origem="imagem" onclick="selecionarProfissional(this);" data-tt="tooltip" title="Espaço Milady" data-original-title="Espaço Milady">
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-header">
              ...
              <div class="card-tools">
                <div class="btn-group">
                  <button type="button" class="btn btn-info btn-sm" id="adicionar_produto_servico"><i class="fas fa-arrow-alt-circle-down"></i></button>
                </div>
              </div>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-4">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control form-control-sm" name="nome_produto_servico" id="nome_produto_servico" readonly="readonly">
                  </div>
                </div>
                <div class="col-2">
                  <div class="form-group">
...
function selecionarProdutoServico( id_prod_serv )
{
  console.log('selecionarProdutoServico')
  pdv_venda_detalhe.id_prod_serv = id_prod_serv;
  ...
  procurar_dados_ProdutoServico( id_prod_serv , false )
}
...
function updateOrCreateVenda()
{
  ...
  axios.post('{{ route('venda.store') }}', pdv_venda)
  .then( function(response)
  {
    ...
    {
      $("#escolher_produto_servico").show();
      $('#id_produto_servico').select2();
    }
  })
@include('includes.catch', [ 'codigo_erro' => '5709039a' ] )
  .then( function()
  {
...
function selecionarProfissional(field)
{
  ...
  url = "{{ route('servico.profServ', [ ':id_servico', ':id_pessoa' ] ) }}";
  url = url.replace(':id_servico', [pdv_venda_detalhe.id_prod_serv] );
  ...
  .then( function(response)
  {
    ...
    fin_conta_interna.status        = 'Em aberto';
...
@include('includes.catch', [ 'codigo_erro' => '2778402a' ] )
  .then( function()
  {
...
$('#adicionar_produto_servico').on('click', function(e)
{
  console.log("'#adicionar_produto_servico').on('click'")
...
@include('includes.catch', [ 'codigo_erro' => '3437904a' ] )
  .then( function() {
    setTimeout(function() {
      $('#overlay_configurar_item').hide();
...
function mostrarItens()
{
  ...
  url = "{{ route('venda.mostrarItens', ':id') }}";
...
@include('includes.catch', [ 'codigo_erro' => '1758119a' ] )
  .then( function() {
    setTimeout(function() {
      $('#overlay_tabela_itens').hide();
    }, 500);
...
function procurar_dados_Cliente()
{
  ...
  //   dados_do_cliente.vendas      = response.data.a_t_d__pessoas__vendas_detalhes;
  //   dados_do_cliente.vendas      = [];
  // })
  {{-- @include('includes.catch', [ 'codigo_erro' => '5081622a' ] ) --}}
  // .then( function()
  // {
...
function procurar_dados_Agendamento()
{
  ...
  //       badge             : value.badge,
  //       nome_profissional : value.kdfalsjdlk_p_r_o_f_i_s_s_i_o_n_a_lasjdlaskjdlkasjd.apelido,
  //       nome_servico      : value.kdfalsjdlk_s_e_r_v_i_c_o_p_r_o_d_u_t_oasjdlaskjdlkasjd.nome,
  //     }
...
function procurar_dados_DetalhesVenda( id_produto_servico )
{
  ...
  url = url.replace(':id', [id_produto_servico] );

  axios.get(url)
}

function procurar_dados_ProdutoServico( id_produto_servico, mostrar )
{
  console.log('procurar_dados_ProdutoServico')
  url_servico = "{{ route('servico.find', ':id') }}";
  url_servico = url_servico.replace(':id', id_produto_servico );
  ...
  .then( function(response)
  {
    ...
    pdv_venda_detalhe.id_prod_serv  = response.data.id;
...
@include('includes.catch', [ 'codigo_erro' => '5075393a' ] )
  .then( function()
  {
    procurar_dados_ProfissionaisCapacitados( id_produto_servico, mostrar )
  })
}

function procurar_dados_ProfissionaisCapacitados( id_produto_servico, mostrar )
{
...
@include('includes.catch', [ 'codigo_erro' => '6805851a' ] )
  .then( function()
  {
  ...
  .then( function()
  {
    preencher_modal_ProdutoServico( id_produto_servico, mostrar )
  })
  .then( function()
  {
  ...
  .then( function()
  {
    ...
    // esc_modal_ProdutoServico( id_produto_servico, mostrar )
  })
  // .then( function()
  // {
  //   $('#escolher_profissional').show();
...
function preencher_tela_ConfigProduto()
{
  ...
  $('#nome_produto_servico').val(pdv_venda_detalhe.nome);
...
function preencher_modal_Cliente()
{
  ...
  //   })
  {{-- @include('includes.catch', [ 'codigo_erro' => '5337788a' ] ) --}}
  // }
  // else
...
function preencher_modal_Agendamento()
{
  ...
  //       '<div class="btn-group">'+
  //       '<a onclick="procurar_dados_ProdutoServico('+value.id_servico+', true)" class="btn btn-default btn-xs"><i class="fa fa-fw fa-search"></i></a>'+
...
function preencher_modal_ProdutoServico( id_produto_servico, mostrar )
{
  console.log('preencher_modal_ProdutoServico')
  // PREENCHER INFORMACOES DO MODAL SOBRE O PRODUTO
  $('#m2_id').val(pdv_venda_detalhe.id_prod_serv);
...
function adicionarNaTela( id_agendamento )
{
  ...
  dados_do_detalhe_da_venda =
  {
    ...
    id_prod_serv     : este_agendamento[0].id_servico,
  ...
  selecionarProdutoServico( dados_do_detalhe_da_venda.id_prod_serv )  
}
...
function adicionarNaComanda( id_agendamento )
{
  ...
  //   // dados_do_cliente.vendas      = response.data.a_t_d__pessoas__vendas_detalhes;
  // })
  {{-- @include('includes.catch', [ 'codigo_erro' => '7299534a' ] ) --}}
  // .then( function()
  // {

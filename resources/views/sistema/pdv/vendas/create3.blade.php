              ...
              <div class="card card-widget widget-user-2">
                <div class="widget-user-header" style="background-color: #dea6c4 !important; padding: 10px;">
                  <div class="widget-user-image">
                    <img class="img-circle info_profile-pic" src="{{ asset('img/Atendimentos/Pessoas/Perfil/0.png') }}" alt="User Avatar">
                  </div>
                  ...
                  <div class="row">
                    <div class="col-2">
                      ...
                      <div class="radio-group">
                        @foreach($profissionais as $profissional)
                        <img src="{{ asset('img/Atendimentos/Pessoas/Perfil/'. $profissional->id .'.png') }}" class="img-circle radio" alt="User Image" width="50px" style="margin: 5px;" id="profissa_{{ $profissional->id }}" data-value="{{ $profissional->id }}" data-origem="imagem" onclick="selecionaProfissional(this);" data-bs-tooltip="tooltip" data-bs-title="" data-original-title="{{ $profissional->apelido }}">
                        @endforeach
                      </div>
                    </div>
...
function validar(field)
{
  ...
  @include('includes.catch', [ 'codigo_erro' => '5205710a' ] )
};
...
@include('includes.catch', [ 'codigo_erro' => '5955396a' ] )
});
</script>
@endsection --}}
...
<script>
  ...
  function selecionarCliente(field) {
    ...
    if(id_cliente != "") {
      ...
      .then( function(response) {
        ...
        if(response.data.a_t_d__pessoas__tipos.filter(x => x.id === 8).map(x => x.nome) == "Influencer Digital")
        {
          desconto_predefinido    = -1;
          tipo_comissao           = "Tabelado";
          selo                    = response.data.a_t_d__pessoas__tipos.filter(x => x.id === 8).map(x => x.nome); 
        }
        else if(response.data.a_t_d__pessoas__tipos.filter(x => x.id === 6).map(x => x.nome) == "Cliente VIP")
        {
          desconto_predefinido    = "VIP";
          tipo_comissao           = "Comissão";
          selo                    = response.data.a_t_d__pessoas__tipos.filter(x => x.id === 6).map(x => x.nome); 
        }
        else if(response.data.a_t_d__pessoas__tipos.filter(x => x.id === 4).map(x => x.nome) == "Colaborador")
        {
          desconto_predefinido    = -0.3;
          tipo_comissao           = "Final";
          selo                    = response.data.a_t_d__pessoas__tipos.filter(x => x.id === 4).map(x => x.nome); 
        }
  ...
  @include('includes.catch', [ 'codigo_erro' => '8191921a' ] )
      .then( function() {
        setTimeout(function() {
          $('#overlay_cliente').hide();
        }, 500);
  ...
  function abrirComanda(id_cliente) {
    ...
    .then( function(response) {
      ...
      if(response.data.data.id != null)
      {
        ...
        // else
        // {
        //   window.location.href= "{{ route('venda.store') }}/"+response.data.data.id+"";
        // }
      }


    })
@include('includes.catch', [ 'codigo_erro' => '4937755a' ] )
  }
...
@include('includes.catch', [ 'codigo_erro' => '6241665a' ] )
    .then( function() {
      setTimeout(function() {
  ...
  // $("#add_serv_prod").on('click', function(e)
  // {
  //   e.preventDefault();
  ...
  //   var id_prod_serv          = $("#id_prod_serv").val();

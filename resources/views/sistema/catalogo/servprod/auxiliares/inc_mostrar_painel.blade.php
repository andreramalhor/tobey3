		...
		<div class="card">
			...
			<div class="card-body p-0">
				<table class="table table-sm">
					...
					<tbody>
						{{--
							@forelse($pessoa->eidwuedesduzkvdendas_detalhes)
							<tr>
		...
		<div class="card">
			...
			<div class="card-body p-0">
				<table class="table table-sm">
					...
					<tbody>
						{{--
							@forelse($pessoa->eidwuedoeduzdsdendas_detalhes)
							<tr>
							<td class="text-nowrap text-left"><a href="" data-bs-toggle="modal" onclick="vendas_mostrar_modal({{ $vendas_detalhes->sbbgaqleesuzlus->id ?? $vendas_detalhes->id_venda ?? 'Erro SHOW.PESSOA 1' }})"><span class="badge bg-pink">{{ $vendas_detalhes->sbbgaqleesuzlus->id ?? $vendas_detalhes->id_venda ?? 'Erro SHOW.PESSOA 1' }}</span></a></td>
							<td class="text-nowrap text-left">{{ \Carbon\Carbon::parse($vendas_detalhes->created_at)->format('d/m/Y') }}</td>
							<td class="text-nowrap text-left">{{ $vendas_detalhes->kcvkongmlqeklsl->nome ?? 'Erro SHOW.PESSOA 4' }}</td>
...
<div class="row">
	<div class="col-4">
		<div class="small-box bg-warning">
			<div class="inner">
			{{-- 
				<h3>{{ $pessoa->eidwuedoeduzdsdedueidwuedoedueidwuedoedueidwuedooduznso:today()) : "99999" }}</h3>
				--}}	
				<p>Dias sem vir</p>
			</div>
			<div class="icon">
	...
	<div class="col-4">
		<div class="small-box bg-success">
			<div class="inner">
				{{--
					<h3>{{ number_format($pessoa->eidwuedoeduzdsd}}</h3>
					--}}
				<p>Faturamento</p>
			</div>
			<div class="icon">

...
<div class="row">
  <div class="card-group">
    ...
    @endif
    <form id="form_servprod_adicionar">
      <div class="col-md-9">
        <div class="row">
          <div class="col-md-12">
  ...
  <div class="row">
    <div class="col-12">
      ...
      <a class="btn btn-success float-right" style="color:white" id="btn_servprod_editar">Editar</a>
    </div>
  </div>
</form>
<br>
...
<!-- 
  `cmv_prod_serv`
  CMV (Custo da Mercadoria Vendida) é a soma de tudo que a empresa gasta para comprar, produzir e estocar seus servprod e
  ...
  seguros, dentre outros gastos.
	...
	`curva_abc`
  A curva ABC tem, como principal finalidade, classificar todos os servprod que compõem o estoque de uma empresa de acordo
...
function validar(item)
{
  ...
  {{--
    var url = "{{ route('cat.servprod.validar', ':idd') }}";
    axios.post('{{ route('cat.servprod.gravar') }}', dados)
    var url = url.replace(':idd', {{ $servprod->id ?? null }});
  --}}
...
@include('includes.catch', [ 'codigo_erro' => '9527753a' ] )
};

$("#btn_servprod_editar").click(function(event)
{
  event.preventDefault();

  var url = "{{ route('cat.servprod.atualizar', [':servprod', ':id']) }}";
  var url = url.replace(':servprod', '{{ $tipo }}');
  var url = url.replace(':id', {{ $servprod->id ?? null }});
  
  let dados = $('#form_servprod_adicionar').serialize();
...
@include('includes.catch', [ 'codigo_erro' => '5565938a' ] )
});


function servprod_avatar_gravar()
{
  const formData = new FormData();
  const imagefile = document.querySelector('#image');
  formData.append("image", imagefile.files[0]);

  let dados = $('#form_servprod_adicionar_avatar');
  // let dados = $('#form_servprod_adicionar_avatar').serialize();
  
  axios.post('{{ route('cat.servprod.avatar', $tipo) }}', formData,
  {
    headers:
    {
...
@include('includes.catch', [ 'codigo_erro' => '3630323a' ] )
  .then()
  {
    $('#imagem_enviar').hide();
...
function servprod_avatar_remove()
{
  var url = "{{ route('cat.servprod.avatar_remove', [':servprod']) }}";
  var url = url.replace(':servprod', '{{ $tipo }}');
...
@include('includes.catch', [ 'codigo_erro' => '4316903a' ] )
  .then()
  {
    $('#imagem_enviar').show();

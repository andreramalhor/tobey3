...
<script type="text/javascript">
	...
	function pessoas_listar()
	{
		...
		.then(function(response)
		{
            ...
            resposta = collect(response.data)
            resposta.each((id, text) => {
            people =
                {
                    id: id,
                    text: text,
...
@include('includes.catch', [ 'codigo_erro' => '7808166a' ] )
		.then( function()
		{
	...
	function pessoa_selecionar(item)
	{
                    ...
                    '<div class="card-body pt-0">'+
                        '<div class="row">'+
                            ...
                            '<div class="col-5 text-center">'+
                                '<img src="'+response.data.foto_perfil+'" alt="'+response.data.nome+'" class="img-circle img-fluid" width="100" onerror="this.src=http://127.0.0.1:8000/img/atendimentos/pessoas/0.png">'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
  	...
  	@include('includes.catch', [ 'codigo_erro' => '5625881a' ] )
		.then( function()
		{
			$('#overlay-modal-usuario-incluir').hide()
		})
...
@include('includes.catch', [ 'codigo_erro' => '5339171a' ] )
		.then(function ()
		{

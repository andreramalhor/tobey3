  ...
  <div class="col-3">
    <div class="card">
      <div class="card-body box-profile">
        <div class="text-center">
          <img class="profile-user-img img-fluid img-circle" src="{{ $mensagem->foto_perfil }}" alt="User profile picture"  data-bs-tooltip="tooltip" data-bs-title="{{ $mensagem->apelido }}">
        </div>
        <h3 class="profile-username text-center">{{ $mensagem->apelido }}</h3>
        <p class="text-muted text-center">{{ $mensagem->nome }}</p>
        <hr>
        <span class="text-center">
          <strong><i class="fas fa-birthday-cake mr-1"></i> Dt Nascimento</strong>
          @if(isset($mensagem->dt_nascimento))
            <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ \Carbon\Carbon::parse($mensagem->dt_nascimento)->format('d/m/Y') }} ({{ \Carbon\Carbon::parse($mensagem->dt_nascimento)->age }} anos)</font></p>
          @else
        ...
        <span class="text-center">
          ...
          <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ $mensagem->cpf ?? "-" }}</font></p>
        </span>
        <span class="text-center">
          <strong><i class="far fa-file-alt mr-1"></i> Observação</strong>
          <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ $mensagem->observacao ?? "-" }}</font></p>
        </span>
        <span class="text-center">
          <strong><i class="far fa-file-alt mr-1"></i> Saldo</strong>
          <p class="text-muted" style="margin-bottom: 8px;"><font size="2,8">{{ number_format($mensagem->saldo_conta ?? 0, 2, ',', '.') }}</font></p>
        </span>
        @can('Equipe.Alterar Senha')
          @if(isset($mensagem->krdhcnrxogfuwla) AND $mensagem->id == Auth::User()->id)
            <a href="{{ route('atd.equipe.alterar_senha', $mensagem->id) }}" class="btn btn-primary btn-block"><b>Alterar Senha</b></a>
          @endif
        @endcan
      </div>
    </div>
    <div class="card card-primary">
      ...
      <div class="card-body">
        ...
        @forelse($mensagem->ginthgfwxbdhwtu->sortbyDesc('whatsapp') as $contato)
          <p class="text-muted" style="margin-bottom: 2px"><font size="2">
            ...
            @if($contato->whatsapp)
            <a class="float-right btn btn-default btn-xs" href="https://api.whatsapp.com/send?phone=55{{ $contato->tellink }}" target="_black" data-bs-tooltip="tooltip" data-bs-title="WhatsApp"><i class="fab fa-whatsapp"></i></a>
            @endif
          </font></p>
        @empty
        ...
        @forelse($mensagem->uqbchiwyagnnkip as $endereco)
          <p class="text-muted" style="margin-bottom: 2px"><font size="2"> 
            <span style="font-size: 7px;"><i class="fas fa-circle fa-xs"></i></span>
        ...
        @if(isset($mensagem->instagram) || isset($mensagem->facebook))
          <p class="text-muted" style="margin-bottom: 2px"><font size="2"> 
            <span style="font-size: 7px;"></span>
            @if($mensagem->instagram)
              <a class="btn btn-default btn-xs" href="https://www.instagram.com/{{ $mensagem->instagram }}" target="_black" data-bs-tooltip="tooltip" data-bs-title="Instagram"><i class="fa-brands fa-instagram"></i></a> {{ $mensagem->instagram }}
            @endif
            <br>
            @if($mensagem->facebook)
              <a class="btn btn-default btn-xs" href="https://www.facebook.com/{{ $mensagem->facebook }}" target="_black" data-bs-tooltip="tooltip" data-bs-title="Facebook"><i class="fa-brands fa-facebook"></i></a> {{ $mensagem->facebook }}
            @endif
        @else
      ...
      <div class="card-body">
        <div class="tab-content">
          <div class=" tab-pane active" id="painel">
            @include('sistema.atendimentos.mensagems.auxiliares.01_inc_mostrar_painel')
          </div>
          <div class="tab-pane" id="sobre">
            @include('sistema.atendimentos.mensagems.auxiliares.02_inc_mostrar_sobre')
          </div>
          <div class="tab-pane" id="agendamentos">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_agendamentos')
          </div>
          <div class="tab-pane" id="financeiro">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_financeiro')
          </div>
          <div class="tab-pane" id="vendas">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_vendas')
          </div>
          <div class="tab-pane" id="tipo">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_tipo')
          </div>
          <div class="tab-pane" id="comissoes_produtos">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_comissoes_p')
          </div>
          <div class="tab-pane" id="comissoes_servicos">
            @include('sistema.atendimentos.mensagems.auxiliares.inc_mostrar_comissoes_s')
          </div>
        </div>
      </div>
    </div>
...
function mostrarVendas(page=1)
{
  ...
  var url = "{{ route('atd.mensagems.vendas', ':idd') }}"
